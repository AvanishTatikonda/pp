...
mydb=mysql.connector.connect(host='localhost',user='root',password='Avanish@1167',db='personal_portfolio')
#mysql=MySQL(app)
Session(app)
@app.route('/index')
def index():
...
#========================================= contact
@app.route('/contact/<dname>/<name>',methods=['GET','POST'])
def contact(dname,name):
    if request.method=="POST":
        ...
        cursor=mydb.cursor(buffered=True)
        cursor.execute("INSERT INTO contact_form (user_id, name, email, phone_number, message) VALUES (%s, %s, %s, %s, %s)", (name, ename, email, phone, message))
        mydb.commit()        #mydb.commit()
        flash('your message submitted sucessfully')
        return redirect(url_for('desig'))
...
def read_contact():
    if session.get('user'):
        ...
        data = cursor.fetchone()
        return render_template('readcontactus.html',i = data) 
    return redirect(url_for('ulogin'))
@app.route('/')
def page():
